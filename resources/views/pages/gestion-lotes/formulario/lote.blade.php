 <div class="row g-3">
    ...
    <div class="col-md-4">
        ...
        <input type="number" step="0.01" name="frente_m" id="frente_m{{$lote->id ?? ''}}" class="form-control" required>
    </div>

    <div class="col-md-4">
        <label for="fondo_m" class="form-label">Fondo (m)</label>
        <input type="number" step="0.01" name="fondo_m" id="fondo_m{{$lote->id ?? ''}}" class="form-control" required>
    </div>
    @if (isset($lote))
        <div class="col-md-4">
            <label for="superficie_m2" class="form-label">Superficie (m²)</label>
            <input type="number" step="0.01" name="superficie_m2" id="superficie_m2{{ $lote->id ?? ''}}" class="form-control">
        </div>        
    @endif

    <div class="col-md-6">
        <label for="precio_contado" class="form-label">Precio Contado</label>
        <input type="number" step="0.01" name="precio_contado" id="precio_contado{{ $lote->id ?? '' }}" class="form-control" required>
    </div>

    <div class="col-md-6">
        <label for="precio_credito" class="form-label">Precio Crédito</label>
        <input type="number" step="0.01" name="precio_credito" id="precio_credito{{ $lote->id ?? '' }}" class="form-control" required>
    </div>

    <div class="col-md-6">
        <label for="plano" class="form-label">Plano (Archivo)</label>
        <input type="file" name="plano" id="plano{{$lote->id ?? ''}}" class="form-control">
    </div>
